...
#include "ggMatrixElements/functions.h"
...
int limits(double sred, double tred, double ured) {
  ...
  else if ((sred <= 10. && -tred < 0.0001 * sred) || (sred > 10. && sred <= shigh && -tred < 0.001) ||
           (sred > shigh && -tred < 1.))
    return forward;  // forward limit
  else if ((sred <= 10. && -ured < 0.0001 * sred) || (sred > 10. && -ured < 0.001) || (sred > shigh && -ured < 1.))
...
void Mxxxx_spin0even(double x, double y, double m, double f0, double w_const, double a2, double* re, double* im) {
  ...
  *re = -4. / (f0 * f0) * x * x /
        ((x - m * m) * (x - m * m) + m * m * width_gen(x, m, f0, w_const, a2) * width_gen(x, m, f0, w_const, a2)) *
        (x - m * m);
  *im = -4. / (f0 * f0) * x * x /
        ((x - m * m) * (x - m * m) + m * m * width_gen(x, m, f0, w_const, a2) * width_gen(x, m, f0, w_const, a2)) *
        (-m * width_gen(x, m, f0, w_const, a2));
}
...
void Mppmm_spin0even(double s, double t, double m, double f0, double w_const, double a2, double* re, double* im) {
  ...
  *re += -4. / (f0 * f0) * s * s /
         ((s - m * m) * (s - m * m) + m * m * width_gen(s, m, f0, w_const, a2) * width_gen(s, m, f0, w_const, a2)) *
         (s - m * m);
  *re += -4. / (f0 * f0) * t * t /
         ((t - m * m) * (t - m * m) + m * m * width_gen(s, m, f0, w_const, a2) * width_gen(s, m, f0, w_const, a2)) *
         (t - m * m);
  *re += -4. / (f0 * f0) * u * u /
         ((u - m * m) * (u - m * m) + m * m * width_gen(s, m, f0, w_const, a2) * width_gen(s, m, f0, w_const, a2)) *
         (u - m * m);

  *im += -4. / (f0 * f0) * s * s /
         ((s - m * m) * (s - m * m) + m * m * width_gen(s, m, f0, w_const, a2) * width_gen(s, m, f0, w_const, a2)) *
         (-m * width_gen(s, m, f0, w_const, a2));
  *im += -4. / (f0 * f0) * t * t /
         ((t - m * m) * (t - m * m) + m * m * width_gen(s, m, f0, w_const, a2) * width_gen(s, m, f0, w_const, a2)) *
         (-m * width_gen(s, m, f0, w_const, a2));
  *im += -4. / (f0 * f0) * u * u /
...
void MZZxxxx_spin0even(
    double x, double y, double m, double f0, double f0Z, double w_const, double a2, double* re, double* im) {
  // some auxiliary function used in Mpppp, Mpmpm, Mpmmp.
  *re = -4. / (f0 * f0Z) * x * (x - 2 * mZ * mZ) /
        ((x - m * m) * (x - m * m) + m * m * width_gen(x, m, f0, w_const, a2) * width_gen(x, m, f0, w_const, a2)) *
        (x - m * m);
  *im = -4. / (f0 * f0Z) * x * (x - 2 * mZ * mZ) /
        ((x - m * m) * (x - m * m) + m * m * width_gen(x, m, f0, w_const, a2) * width_gen(x, m, f0, w_const, a2)) *
        (-m * width_gen(x, m, f0, w_const, a2));
}

void MZZpppp_spin0even(
    double s, double t, double m, double f0, double f0Z, double w_const, double a2, double* re, double* im) {
  MZZxxxx_spin0even(s, t, m, f0, f0Z, w_const, a2, re, im);
}

void MZZpmmp_spin0even(
    double s, double t, double m, double f0, double f0Z, double w_const, double a2, double* re, double* im) {
  MZZxxxx_spin0even(t, s, m, f0, f0Z, w_const, a2, re, im);
}

void MZZpmpm_spin0even(
    double s, double t, double m, double f0, double f0Z, double w_const, double a2, double* re, double* im) {
  double u = -s - t;
  MZZxxxx_spin0even(u, t, m, f0, f0Z, w_const, a2, re, im);
}

void MZZppmm_spin0even(
    double s, double t, double m, double f0, double f0Z, double w_const, double a2, double* re, double* im) {
  double u = -s - t;
  *re = *im = 0.;

  *re += -4. / (f0 * f0Z) * s * (s - 2 * mZ * mZ) /
         ((s - m * m) * (s - m * m) + m * m * width_gen(s, m, f0, w_const, a2) * width_gen(s, m, f0, w_const, a2)) *
         (s - m * m);
  *re += -4. / (f0 * f0Z) * t * (t - 2 * mZ * mZ) /
         ((t - m * m) * (t - m * m) + m * m * width_gen(t, m, f0, w_const, a2) * width_gen(t, m, f0, w_const, a2)) *
         (t - m * m);
  *re += -4. / (f0 * f0Z) * u * (u - 2 * mZ * mZ) /
         ((u - m * m) * (u - m * m) + m * m * width_gen(u, m, f0, w_const, a2) * width_gen(u, m, f0, w_const, a2)) *
         (u - m * m);

  *im += -4. / (f0 * f0Z) * s * (s - 2 * mZ * mZ) /
         ((s - m * m) * (s - m * m) + m * m * width_gen(s, m, f0, w_const, a2) * width_gen(s, m, f0, w_const, a2)) *
         (-m * width_gen(s, m, f0, w_const, a2));
  *im += -4. / (f0 * f0Z) * t * (t - 2 * mZ * mZ) /
         ((t - m * m) * (t - m * m) + m * m * width_gen(t, m, f0, w_const, a2) * width_gen(t, m, f0, w_const, a2)) *
         (-m * width_gen(t, m, f0, w_const, a2));
  *im += -4. / (f0 * f0Z) * u * (u - 2 * mZ * mZ) /
         ((u - m * m) * (u - m * m) + m * m * width_gen(u, m, f0, w_const, a2) * width_gen(u, m, f0, w_const, a2)) *
         (-m * width_gen(u, m, f0, w_const, a2));

  //std::cout <<"Mppmm " <<"\t" << *re  << std::endl;
}

void MZZpppm_spin0even(
    double s, double t, double m, double f0, double f0Z, double w_const, double a2, double* re, double* im) {
  *re = *im = 0.;
}

void MZZpp00_spin0even(
    double s, double t, double m, double f0, double f0Z, double w_const, double a2, double* re, double* im) {
  double u = -s - t;
  *re = *im = 0;

  *re += -4. / (f0 * f0Z) * s * (2 * mZ * mZ) /
         ((s - m * m) * (s - m * m) + m * m * width_gen(s, m, f0, w_const, a2) * width_gen(s, m, f0, w_const, a2)) *
         (s - m * m);
  *re += -4. / (f0 * f0Z) * t * (2 * mZ * mZ) /
         ((t - m * m) * (t - m * m) + m * m * width_gen(t, m, f0, w_const, a2) * width_gen(t, m, f0, w_const, a2)) *
         (t - m * m);
  *re += -4. / (f0 * f0Z) * u * (2 * mZ * mZ) /
         ((u - m * m) * (u - m * m) + m * m * width_gen(u, m, f0, w_const, a2) * width_gen(u, m, f0, w_const, a2)) *
         (u - m * m);

  *im += -4. / (f0 * f0Z) * s * (2 * mZ * mZ) /
         ((s - m * m) * (s - m * m) + m * m * width_gen(s, m, f0, w_const, a2) * width_gen(s, m, f0, w_const, a2)) *
         (-m * width_gen(s, m, f0, w_const, a2));
  *im += -4. / (f0 * f0Z) * t * (2 * mZ * mZ) /
         ((t - m * m) * (t - m * m) + m * m * width_gen(t, m, f0, w_const, a2) * width_gen(t, m, f0, w_const, a2)) *
         (-m * width_gen(t, m, f0, w_const, a2));
  *im += -4. / (f0 * f0Z) * u * (2 * mZ * mZ) /
         ((u - m * m) * (u - m * m) + m * m * width_gen(u, m, f0, w_const, a2) * width_gen(u, m, f0, w_const, a2)) *
         (-m * width_gen(u, m, f0, w_const, a2));
}
...
void MZgxxxx_spin0even(
    double x, double y, double m, double f0, double f0Zg, double w_const, double a2, double* re, double* im) {
  // some auxiliary function used in Mpppp, Mpmpm, Mpmmp.
  *re = -4. / (f0 * f0Zg) * x * (x - mZ * mZ) /
        ((x - m * m) * (x - m * m) + m * m * width_gen(x, m, f0, w_const, a2) * width_gen(x, m, f0, w_const, a2)) *
        (x - m * m);
  *im = -4. / (f0 * f0Zg) * x * (x - mZ * mZ) /
        ((x - m * m) * (x - m * m) + m * m * width_gen(x, m, f0, w_const, a2) * width_gen(x, m, f0, w_const, a2)) *
        (-m * width_gen(x, m, f0, w_const, a2));
}

void MZgpppp_spin0even(
    double s, double t, double m, double f0, double f0Zg, double w_const, double a2, double* re, double* im) {
  MZgxxxx_spin0even(s, t, m, f0, f0Zg, w_const, a2, re, im);
}

void MZgpmmp_spin0even(
    double s, double t, double m, double f0, double f0Zg, double w_const, double a2, double* re, double* im) {
  MZgxxxx_spin0even(t, s, m, f0, f0Zg, w_const, a2, re, im);
}

void MZgpmpm_spin0even(
    double s, double t, double m, double f0, double f0Zg, double w_const, double a2, double* re, double* im) {
  double u = -s - t;
  MZgxxxx_spin0even(u, t, m, f0, f0Zg, w_const, a2, re, im);
}

void MZgppmm_spin0even(
    double s, double t, double m, double f0, double f0Zg, double w_const, double a2, double* re, double* im) {
  double u = -s - t;
  *re = *im = 0.;

  *re += -4. / (f0 * f0Zg) * s * (s - mZ * mZ) /
         ((s - m * m) * (s - m * m) + m * m * width_gen(s, m, f0, w_const, a2) * width_gen(s, m, f0, w_const, a2)) *
         (s - m * m);
  *re += -4. / (f0 * f0Zg) * t * (t - mZ * mZ) /
         ((t - m * m) * (t - m * m) + m * m * width_gen(t, m, f0, w_const, a2) * width_gen(t, m, f0, w_const, a2)) *
         (t - m * m);
  *re += -4. / (f0 * f0Zg) * u * (u - mZ * mZ) /
         ((u - m * m) * (u - m * m) + m * m * width_gen(u, m, f0, w_const, a2) * width_gen(u, m, f0, w_const, a2)) *
         (u - m * m);

  *im += -4. / (f0 * f0Zg) * s * (s - mZ * mZ) /
         ((s - m * m) * (s - m * m) + m * m * width_gen(s, m, f0, w_const, a2) * width_gen(s, m, f0, w_const, a2)) *
         (-m * width_gen(s, m, f0, w_const, a2));
  *im += -4. / (f0 * f0Zg) * t * (t - mZ * mZ) /
         ((t - m * m) * (t - m * m) + m * m * width_gen(t, m, f0, w_const, a2) * width_gen(t, m, f0, w_const, a2)) *
         (-m * width_gen(t, m, f0, w_const, a2));
  *im += -4. / (f0 * f0Zg) * u * (u - mZ * mZ) /
         ((u - m * m) * (u - m * m) + m * m * width_gen(u, m, f0, w_const, a2) * width_gen(u, m, f0, w_const, a2)) *
         (-m * width_gen(u, m, f0, w_const, a2));
  //std::cout <<"Mppmm " <<"\t" << *re  << std::endl;
}

void MZgpppm_spin0even(
    double s, double t, double m, double f0, double f0Zg, double w_const, double a2, double* re, double* im) {
  *re = *im = 0.;
}

void MZgpp00_spin0even(
    double s, double t, double m, double f0, double f0Zg, double w_const, double a2, double* re, double* im) {
  *re = *im = 0.;
}
...
void MWWxxxx_spin0even(
    double x, double y, double m, double f0, double f0W, double w_const, double a2, double* re, double* im) {
  // some auxiliary function used in Mpppp, Mpmpm, Mpmmp.
  *re = -4. / (f0 * f0W) * x * (x - 2 * mW * mW) /
        ((x - m * m) * (x - m * m) + m * m * width_gen(x, m, f0, w_const, a2) * width_gen(x, m, f0, w_const, a2)) *
        (x - m * m);
  *im = -4. / (f0 * f0W) * x * (x - 2 * mW * mW) /
        ((x - m * m) * (x - m * m) + m * m * width_gen(x, m, f0, w_const, a2) * width_gen(x, m, f0, w_const, a2)) *
        (-m * width_gen(x, m, f0, w_const, a2));
}

void MWWpppp_spin0even(
    double s, double t, double m, double f0, double f0W, double w_const, double a2, double* re, double* im) {
  MWWxxxx_spin0even(s, t, m, f0, f0W, w_const, a2, re, im);
}

void MWWpmmp_spin0even(
    double s, double t, double m, double f0, double f0W, double w_const, double a2, double* re, double* im) {
  MWWxxxx_spin0even(t, s, m, f0, f0W, w_const, a2, re, im);
}

void MWWpmpm_spin0even(
    double s, double t, double m, double f0, double f0W, double w_const, double a2, double* re, double* im) {
  double u = -s - t;
  MWWxxxx_spin0even(u, t, m, f0, f0W, w_const, a2, re, im);
}

void MWWppmm_spin0even(
    double s, double t, double m, double f0, double f0W, double w_const, double a2, double* re, double* im) {
  double u = -s - t;
  *re = *im = 0.;

  *re += -4. / (f0 * f0W) * s * (s - 2 * mW * mW) /
         ((s - m * m) * (s - m * m) + m * m * width_gen(s, m, f0, w_const, a2) * width_gen(s, m, f0, w_const, a2)) *
         (s - m * m);
  *re += -4. / (f0 * f0W) * t * (t - 2 * mW * mW) /
         ((t - m * m) * (t - m * m) + m * m * width_gen(t, m, f0, w_const, a2) * width_gen(t, m, f0, w_const, a2)) *
         (t - m * m);
  *re += -4. / (f0 * f0W) * u * (u - 2 * mW * mW) /
         ((u - m * m) * (u - m * m) + m * m * width_gen(u, m, f0, w_const, a2) * width_gen(u, m, f0, w_const, a2)) *
         (u - m * m);

  *im += -4. / (f0 * f0W) * s * (s - 2 * mW * mW) /
         ((s - m * m) * (s - m * m) + m * m * width_gen(s, m, f0, w_const, a2) * width_gen(s, m, f0, w_const, a2)) *
         (-m * width_gen(s, m, f0, w_const, a2));
  *im += -4. / (f0 * f0W) * t * (t - 2 * mW * mW) /
         ((t - m * m) * (t - m * m) + m * m * width_gen(t, m, f0, w_const, a2) * width_gen(t, m, f0, w_const, a2)) *
         (-m * width_gen(t, m, f0, w_const, a2));
  *im += -4. / (f0 * f0W) * u * (u - 2 * mW * mW) /
         ((u - m * m) * (u - m * m) + m * m * width_gen(u, m, f0, w_const, a2) * width_gen(u, m, f0, w_const, a2)) *
         (-m * width_gen(u, m, f0, w_const, a2));
...
void MWWpppm_spin0even(
    double s, double t, double m, double f0, double f0W, double w_const, double a2, double* re, double* im) {
  *re = *im = 0;
}

void MWWpp00_spin0even(
    double s, double t, double m, double f0, double f0W, double w_const, double a2, double* re, double* im) {
  double u = -s - t;
  *re = *im = 0.;

  *re += -4. / (f0 * f0W) * s * (2 * mW * mW) /
         ((s - m * m) * (s - m * m) + m * m * width_gen(s, m, f0, w_const, a2) * width_gen(s, m, f0, w_const, a2)) *
         (s - m * m);
  *re += -4. / (f0 * f0W) * t * (2 * mW * mW) /
         ((t - m * m) * (t - m * m) + m * m * width_gen(t, m, f0, w_const, a2) * width_gen(t, m, f0, w_const, a2)) *
         (t - m * m);
  *re += -4. / (f0 * f0W) * u * (2 * mW * mW) /
         ((u - m * m) * (u - m * m) + m * m * width_gen(u, m, f0, w_const, a2) * width_gen(u, m, f0, w_const, a2)) *
         (u - m * m);

  *im += -4. / (f0 * f0W) * s * (2 * mW * mW) /
         ((s - m * m) * (s - m * m) + m * m * width_gen(s, m, f0, w_const, a2) * width_gen(s, m, f0, w_const, a2)) *
         (-m * width_gen(s, m, f0, w_const, a2));
  *im += -4. / (f0 * f0W) * t * (2 * mW * mW) /
         ((t - m * m) * (t - m * m) + m * m * width_gen(t, m, f0, w_const, a2) * width_gen(t, m, f0, w_const, a2)) *
         (-m * width_gen(t, m, f0, w_const, a2));
  *im += -4. / (f0 * f0W) * u * (2 * mW * mW) /
         ((u - m * m) * (u - m * m) + m * m * width_gen(u, m, f0, w_const, a2) * width_gen(u, m, f0, w_const, a2)) *
         (-m * width_gen(u, m, f0, w_const, a2));
...
void Mxxxx_spin2(double x, double y, double m, double f0, double w_const, double a2, double* re, double* im) {
  ...
  *re += -1. / (f0 * f0) * x * x /
         ((y - m * m) * (y - m * m) + m * m * width_gen(y, m, f0, w_const, a2) * width_gen(y, m, f0, w_const, a2)) *
         (y - m * m);
  *im += -1. / (f0 * f0) * x * x /
         ((y - m * m) * (y - m * m) + m * m * width_gen(y, m, f0, w_const, a2) * width_gen(y, m, f0, w_const, a2)) *
         (-m * width_gen(y, m, f0, w_const, a2));
  *re += -1. / (f0 * f0) * x * x /
         ((z - m * m) * (z - m * m) + m * m * width_gen(z, m, f0, w_const, a2) * width_gen(z, m, f0, w_const, a2)) *
         (z - m * m);
  *im += -1. / (f0 * f0) * x * x /
         ((z - m * m) * (z - m * m) + m * m * width_gen(z, m, f0, w_const, a2) * width_gen(z, m, f0, w_const, a2)) *
         (-m * width_gen(z, m, f0, w_const, a2));
}
...
void Mggxxxx_spin0even(
    double x, double y, double m, double f0, double f0g, double w_const, double a2, double* re, double* im) {
  // some auxiliary function used in Mpppp, Mpmpm, Mpmmp.
  *re = -4. / (f0 * f0g) * x * x /
        ((x - m * m) * (x - m * m) + m * m * width_gen(x, m, f0, w_const, a2) * width_gen(x, m, f0, w_const, a2)) *
        (x - m * m);
...
void Mggpppp_spin0even(
    double s, double t, double m, double f0, double f0g, double w_const, double a2, double* re, double* im) {
  Mggxxxx_spin0even(s, t, m, f0, f0g, w_const, a2, re, im);
}

void Mggpmmp_spin0even(
    double s, double t, double m, double f0, double f0g, double w_const, double a2, double* re, double* im) {
  *re = *im = 0.;
}

void Mggpmpm_spin0even(
    double s, double t, double m, double f0, double f0g, double w_const, double a2, double* re, double* im) {
  *re = *im = 0.;
}

void Mggppmm_spin0even(
    double s, double t, double m, double f0, double f0g, double w_const, double a2, double* re, double* im) {
  *re = -4. / (f0 * f0g) * s * s /
        ((s - m * m) * (s - m * m) + m * m * width_gen(s, m, f0, w_const, a2) * width_gen(s, m, f0, w_const, a2)) *
        (s - m * m);
...
void MhhOHxxxx_spin0even(
    double x, double y, double m, double f0, double f0H, double w_const, double a2, double* re, double* im) {
  *re = 4. / (f0 * f0H) * x * (mh * mh) /
        ((x - m * m) * (x - m * m) + m * m * width_gen(x, m, f0, w_const, a2) * width_gen(x, m, f0, w_const, a2)) *
        (x - m * m);
  *im = 4. / (f0 * f0H) * x * (mh * mh) /
...
void MhhOHpppp_spin0even(
    double s, double t, double m, double f0, double f0H, double w_const, double a2, double* re, double* im) {
  MhhOHxxxx_spin0even(s, t, m, f0, f0H, w_const, a2, re, im);
}

void MhhOHpmmp_spin0even(
    double s, double t, double m, double f0, double f0H, double w_const, double a2, double* re, double* im) {
  *re = *im = 0.;
}

void MhhOHpmpm_spin0even(
    double s, double t, double m, double f0, double f0H, double w_const, double a2, double* re, double* im) {
  *re = *im = 0.;
}

void MhhOHppmm_spin0even(
    double s, double t, double m, double f0, double f0H, double w_const, double a2, double* re, double* im) {
  *re = 4. / (f0 * f0H) * s * (mh * mh) /
        ((s - m * m) * (s - m * m) + m * m * width_gen(s, m, f0, w_const, a2) * width_gen(s, m, f0, w_const, a2)) *
        (s - m * m);
...
void MhhOHpppm_spin0even(
    double s, double t, double m, double f0, double f0H, double w_const, double a2, double* re, double* im) {
  *re = *im = 0.;
}

void MhhOHpp00_spin0even(
    double s, double t, double m, double f0, double f0H, double w_const, double a2, double* re, double* im) {
  *re = 2. / (f0 * f0H) * s * (s - 2 * mh * mh) /
        ((s - m * m) * (s - m * m) + m * m * width_gen(s, m, f0, w_const, a2) * width_gen(s, m, f0, w_const, a2)) *
        (s - m * m);
  *im = 2. / (f0 * f0H) * s * (s - 2 * mh * mh) /
        ((s - m * m) * (s - m * m) + m * m * width_gen(s, m, f0, w_const, a2) * width_gen(s, m, f0, w_const, a2)) *
        (-m * width_gen(s, m, f0, w_const, a2));
}
...
void Mpppm_eft_AZ(double zeta1, double s, double t, double* re, double* im) { *re = *im = 0.; }

/// Z Z final state from OH operator

void MZZOHxxxx_spin0even(
    double x, double y, double m, double f0, double f0H, double w_const, double a2, double* re, double* im) {
  *re = 4. / (f0 * f0H) * x * (mZ * mZ) /
        ((x - m * m) * (x - m * m) + m * m * width_gen(x, m, f0, w_const, a2) * width_gen(x, m, f0, w_const, a2)) *
        (x - m * m);
...
void MZZOHpppp_spin0even(
    double s, double t, double m, double f0, double f0H, double w_const, double a2, double* re, double* im) {
  MZZOHxxxx_spin0even(s, t, m, f0, f0H, w_const, a2, re, im);
}

void MZZOHpmmp_spin0even(
    double s, double t, double m, double f0, double f0H, double w_const, double a2, double* re, double* im) {
  *re = *im = 0.;
}

void MZZOHpmpm_spin0even(
    double s, double t, double m, double f0, double f0H, double w_const, double a2, double* re, double* im) {
  *re = *im = 0.;
}

void MZZOHppmm_spin0even(
    double s, double t, double m, double f0, double f0H, double w_const, double a2, double* re, double* im) {
  *re = 4. / (f0 * f0H) * s * (mZ * mZ) /
        ((s - m * m) * (s - m * m) + m * m * width_gen(s, m, f0, w_const, a2) * width_gen(s, m, f0, w_const, a2)) *
        (s - m * m);
...
void MZZOHpppm_spin0even(
    double s, double t, double m, double f0, double f0H, double w_const, double a2, double* re, double* im) {
  *re = *im = 0.;
}

void MZZOHpp00_spin0even(
    double s, double t, double m, double f0, double f0H, double w_const, double a2, double* re, double* im) {
  *re = 2. / (f0 * f0H) * s * (s - 2 * mZ * mZ) /
        ((s - m * m) * (s - m * m) + m * m * width_gen(s, m, f0, w_const, a2) * width_gen(s, m, f0, w_const, a2)) *
        (s - m * m);
  *im = 2. / (f0 * f0H) * s * (s - 2 * mZ * mZ) /
        ((s - m * m) * (s - m * m) + m * m * width_gen(s, m, f0, w_const, a2) * width_gen(s, m, f0, w_const, a2)) *
        (-m * width_gen(s, m, f0, w_const, a2));
}
